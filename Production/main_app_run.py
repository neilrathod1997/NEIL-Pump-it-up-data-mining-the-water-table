...
processed_test = final_fun_1(data)

@app.route('/pred')
def predict():
    clf1 = load('clf.joblib')
    pred = clf1.predict(processed_test)
    ...
    if val == 1:
        return "Pump is functional but need repairs"
    
    if val == 2:
        return "Pump is Not functional"
    return str(pred)
